...
void   flux_x(double ** h_b, double ** restrict fl_x, int nx, int ny, int j_st ,int j_en ) { 
  int ix , iy;
...
void   flux_y(double ** h_b, double ** restrict fl_y, int nx, int ny, int j_st ,int j_en )   
{
  int ix , iy, j_start,j_end;

  j_start = j_st ; if(j_st < 1)    j_start = 1 ;
  j_end   = j_en ; if(j_en > ny-2) j_end   = ny-2 ;
