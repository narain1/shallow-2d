...
#include <math.h>
#include <time.h>
#include <omp.h>  // uncomment  this header
#include "flux.h"
#include "tendency.h"
#include "bound.h"
...
int main()
{
  ...
  /************************************************/
  FILE  *file_out;
  int it,ir,ix,iy,ixs,iys,nproc,ip;
  double xx,yy,at;
  int irec ;
  clock_t tic , toc;
  /************************************************/
  ...
  nproc = omp_get_max_threads(); // needs to be commented after parallelezation


  int *j_st  = (int *) malloc( nproc* sizeof(int));
  int *j_en  = (int *) malloc( nproc* sizeof(int));
  int *j_dim = (int *) malloc( nproc* sizeof(int));
  ...
  #pragma omp parallel for private(xx, yy, iy, ix)
  for(  iy = 0; iy <= ny-1;  iy++ ) {
    for(  ix = 0; ix <= nx-1;  ix++ ){
  ...
  for(  it = 1; it <= nt;  it++ ) { /* start rk3 integration */
    for(  ir = 1; ir <= nrk3;  ir++ ) {  /* start rk3 substeb */
      ...
      Note:  The following code needs to be reodrered 
      and the subrotine dudx_at_h, dvdy_at_h,dhdx_at_u,dhdy_at_v
      ...
      for (ip = 0; ip < nproc; ip++) {
          ...
          int j_st_local = j_st[ip];
          int j_en_local = j_en[ip];
          flux_x(u_b, fl_x, nx, ny, j_st_local, j_en_local);
          dudx_at_h(fl_x, dudx, dx, nx, ny, j_st_local, j_en_local);
          // dudx_at_h(fl_x, dudx, dx, nx, ny);

          flux_y(v_b, fl_y, nx, ny, j_st_local, j_en_local);
          dvdy_at_h(fl_y, dvdy, dy, nx, ny, j_st_local, j_en_local);
          // dvdy_at_h(fl_y, dvdy, dy, nx, ny);

          flux_x(h_b, fl_x, nx, ny, j_st_local, j_en_local);
          dhdx_at_u(fl_x, dhdx, dx, nx, ny, j_st_local, j_en_local);
          // dhdx_at_u(fl_x, dhdx, dx, nx, ny);

          flux_y(h_b, fl_y, nx, ny, j_st_local, j_en_local);
          dhdy_at_v(fl_y, dhdy, dy, nx, ny, j_st_local, j_en_local);
